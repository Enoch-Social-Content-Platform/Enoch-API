...
export class UserInfoResolver {
    ...
    async getUserInterests(
        ...
        const followers = await getConnection()
            ...
            // .leftJoinAndSelect('user.following', 'follower', "follower.follower = user.id")
            .where('follower.followerId = :followerId', { followerId: payload.userId })
            .getMany();
        for (var follow = 0; follow < followers.length; follow++) {
        }
        const qb = await getConnection()
            .getRepository(User)
            .createQueryBuilder('user')
